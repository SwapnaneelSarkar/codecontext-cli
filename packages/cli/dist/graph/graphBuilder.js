...
function moduleLabelForPath(filePath) {
 ...
 */
async function buildDependencyGraph(fileContexts, dependencyMap) {
    const nodes = [];
    const edges = [];
    const now = new Date().toISOString();
    for (const context of fileContexts) {
        const node = {
            id: (0, core_1.generateId)(context.filePath),
            type: 'file',
    ...
    for (const context of fileContexts) {
        ...
        if (!resolved)
            continue;
        const fromId = (0, core_1.generateId)(context.filePath);
        for (const target of resolved.resolved) {
            const toId = (0, core_1.generateId)(target);
            edges.push({
                source: fromId,
                target: toId,
                relation: 'imports',
    ...
    for (const c of fileContexts) {
        ...
        arr.push((0, core_1.generateId)(c.filePath));
        moduleClusters.set(ml, arr);
    }
    for (const [name, ids] of moduleClusters) {
        if (ids.length < 2)
            continue;
        const id = `module_${(0, core_1.generateId)(name)}`;
        nodes.push({
            id,
            type: 'module',
            label: name,
    ...
    for (const c of fileContexts) {
        for (const d of c.dbAccess) {
            const id = `db_${(0, core_1.generateId)(d)}`;
            if (!dbIds.has(id)) {
                dbIds.add(id);
                nodes.push({
                    id,
            ...
            edges.push({
                source: (0, core_1.generateId)(c.filePath),
                target: id,
                relation: 'reads_from',
            });
        }
        for (const a of c.apiUsage) {
            const id = `api_${(0, core_1.generateId)(a)}`;
            if (!apiIds.has(id)) {
                apiIds.add(id);
                nodes.push({
                    id,

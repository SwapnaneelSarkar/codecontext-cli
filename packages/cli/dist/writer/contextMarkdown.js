...
exports.buildFileSummaryFromContext = buildFileSummaryFromContext;
exports.buildContextMd = buildContextMd;
exports.buildClaudeMemoryMd = buildClaudeMemoryMd;
 ...
 */
function buildFileSummaryFromContext(fc) {
    const lang = fc.language;
    const exportNames = fc.exports.filter(Boolean);
    let exportPhrase = exportNames.length === 0
    ...
    if (lang === 'Dart' && exportNames.length > 0) {
        const widgetKind = fc.apiUsage.find((a) => a === 'flutter:StatefulWidget' || a === 'flutter:StatelessWidget');
        if (widgetKind) {
    ...
    const deps = [...new Set(fc.dependencies)]
        ...
        .slice(0, 3);
    const depPhrase = deps.length > 0 ? deps.join(', ') : 'local/relative paths only';
...
function moduleDescription(files) {
    ...
    const widgets = files.filter((f) => f.apiUsage.some((a) => a.startsWith('flutter:') && a.includes('Widget'))).length;
    const screensHint = files.filter((f) => /screen/i.test(f.filePath)).length;
    const parts = [`${files.length} ${langsStr} file(s)`];
    if (widgets)
        parts.push(`${widgets} with Flutter widgets`);
    if (screensHint)
        parts.push(`${screensHint} path(s) suggest UI/screens`);
    return parts.join('; ') + '.';
}
function buildTwoLevelTree(files) {
    const tree = new Map();
...
function fileImportFanIn(graph) {
    const fanIn = new Map();
    for (const n of graph.nodes) {
        if (n.filePath)
            fanIn.set(n.filePath, 0);
    }
...
function scoreKeyFile(fc, fanIn) {
    return (fanIn * 1000 +
        fc.functions.length * 10 +
        fc.exports.length * 5 +
        fc.imports.length);
}
function aggregateApiLabels(files) {
    const s = new Set();
    for (const f of files) {
        for (const a of f.apiUsage)
            s.add(a);
    }
    return s;
}
function aggregateDbLabels(files) {
    const s = new Set();
    for (const f of files) {
        for (const d of f.dbAccess)
            s.add(d);
    }
    return s;
}
function buildContextMd(projectName, files, graph) {
    ...
    const moduleEntries = [...byModule.entries()].sort((a, b) => b[1].length - a[1].length);
    ...
    for (const e of graph.edges) {
        ...
        if (sm === tm)
            continue;
        if (!moduleEdges.has(sm))
            moduleEdges.set(sm, new Map());
        const inner = moduleEdges.get(sm);
        inner.set(tm, (inner.get(tm) ?? 0) + 1);
    }
    const apiLabels = aggregateApiLabels(files);
    const stateMgmt = [...apiLabels].filter((a) => a.includes('provider') ||
        a.includes('riverpod') ||
        a.includes('bloc') ||
        a.includes('getx') ||
        (a.startsWith('flutter:') && a.includes('Widget')));
    const httpLabels = [...apiLabels].filter((a) => a === 'dart:http' ||
        a === 'dart:dio' ||
        a.includes('GoRouter') ||
    ...
    for (const f of keyFiles) {
        ...
        const classes = f.exports.length > 0
            ? f.exports.slice(0, 12).map((x) => `\`${x}\``).join(', ')
            : '—';
        const funcs = f.functions.length > 0
            ? f.functions.slice(0, 10).map((x) => `\`${x}\``).join(', ')
            : '—';
        lines.push(`### \`${f.filePath}\``);
        lines.push('');
        lines.push(`**Imported by ~${fi} other file(s)** · ${f.functions.length} functions · ${f.exports.length} exports`);
        lines.push('');
...
function findEntrypoints(files) {
    ...
    for (const f of files) {
        ...
        if (p.endsWith('main.dart'))
            paths.add(p);
        if (/router|routes|navigation/i.test(p) && p.endsWith('.dart'))
            paths.add(p);
        if (f.apiUsage.includes('flutter:GoRouter'))
            paths.add(p);
    }
    return [...paths].sort();
}
function inferProjectPurpose(files, projectName) {
...
function buildClaudeMemoryMd(projectName, files, graph) {
    const languages = [...new Set(files.map((f) => f.language))].sort().join(', ');
    ...
    if (entrypoints.length) {
        ...
        lines.push('- No `main.dart` / obvious router path matched; search for `main(` or your framework’s bootstrap file.');
    }
    lines.push('');
    lines.push('## State management (heuristic)', '');
    if (stateLines.length === 0) {
        lines.push('No strong Provider/Riverpod/Bloc/GetX signals in the heuristic pass. Inspect `lib/` for `ChangeNotifier`, `Bloc`, `ref.watch`, etc.');
    }
    else {
    ...
    lines.push('');
    lines.push('## How to navigate', '', '1. Open **Key Files** in `.ai-context/CONTEXT.md` (highest fan-in + size heuristics).', '2. For any file, read `.ai-context/files/<slug>.json` for imports, functions, and `apiUsage`/`dbAccess` tags.', '3. **Dependency graph:** `.ai-context/graph.json` (nodes = files, edges = resolved imports).', '4. **Full index:** `.ai-context/index.json` lists all files and languages.', '', '### Highest-impact paths (start here)', '', ...topKey.map((p) => `- \`${p}\``), '');
    return lines.join('\n');
}
//# sourceMappingURL=contextMarkdown.js.map
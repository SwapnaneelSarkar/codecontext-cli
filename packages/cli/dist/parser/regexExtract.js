...
exports.extractDart = extractDart;
exports.extractJsTsLike = extractJsTsLike;
...
function dartDependencyKey(spec) {
    const s = spec.trim();
...
function extractDart(source) {
    const importDetails = [];
    const exports = [];
    const functions = [];
    const dependencies = new Set();
    const pushImp = (spec, isDynamic) => {
        const s = spec.trim();
    ...
    while ((m = classRe.exec(source)))
        exports.push(m[1]);
    while ((m = mixinRe.exec(source)))
        exports.push(m[1]);
    while ((m = extRe.exec(source)))
        exports.push(m[1]);
    const voidFn = /\bvoid\s+(\w+)\s*\(/g;
    while ((m = voidFn.exec(source))) {
        if (!DART_STMT_RESERVED.has(m[1]))
            functions.push(m[1]);
    }
    ...
    const lineDecl = /^\s*(?:@\w+(?:\([^)]*\))?\s*)*(?:static\s+)?(?:async\s+)?(?:void|Future(?:<[^>]+>)?|Stream(?:<[^>]+>)?|[A-Za-z_]\w*(?:<[^>]*>)?)\s+(\w+)\s*\(/gm;
    while ((m = lineDecl.exec(source))) {
        const name = m[1];
        if (!DART_STMT_RESERVED.has(name))
            functions.push(name);
    }
    ...
    for (let i = 0; i < lines.length; i++) {
        if (!/^\s*@override\b/.test(lines[i]))
            continue;
        for (let j = i + 1; j < Math.min(i + 8, lines.length); j++) {
            const line = lines[j];
            if (/^\s*$/.test(line))
                continue;
            const mm = line.match(/^\s*(?:@\w+(?:\([^)]*\))?\s*)*(?:static\s+)?(?:async\s+)?(?:void|Future(?:<[^>]+>)?|[A-Za-z_]\w*(?:<[^>]*>)?)\s+(\w+)\s*\(/);
...
function extractJsTsLike(source) {
    const importDetails = [];
    const exports = [];
    const functions = [];
    const dependencies = new Set();
    ...
    const exportConstFn = /export\s+const\s+(\w+)\s*=\s*(?:async\s*)?\(/g;
    ...
    while ((m = exportNamed.exec(source)))
        exports.push(m[1]);
    while ((m = exportConstFn.exec(source)))
        exports.push(m[1]);
    while ((m = exportDefault.exec(source)))
        exports.push(m[1]);
    while ((m = exportClass.exec(source)))
        exports.push(m[1]);

...
exports.detectDbAccess = detectDbAccess;
exports.detectApiUsage = detectApiUsage;
exports.detectDartFlutterContext = detectDartFlutterContext;
/**
 * DB / API hints from source text (complements AST where available)
 */
function detectDbAccess(source, lang) {
    const found = new Set();
    const patterns = [
        [/prisma\.\w+/gi, 'prisma'],
    ...
    if (lang === 'Dart') {
        patterns.push([/FirebaseFirestore|cloud_firestore|FirebaseDatabase/i, 'firebase_firestore'], [/FirebaseAuth|firebase_auth/i, 'firebase_auth'], [/sqflite|openDatabase\s*\(/i, 'sqflite_sqlite'], [/shared_preferences|SharedPreferences/i, 'shared_preferences'], [/hive\.|Isar\.|isar|drift\.|moor/i, 'local_db_package'], [/secure_storage|FlutterSecureStorage/i, 'secure_storage']);
    }
...
function detectApiUsage(source, lang) {
    const found = new Set();
...
function detectDartFlutterContext(source) {
    ...
    if ((/\.fromJson|factory\s+\w+\.fromJson/i.test(source) && /toJson\s*\(/i.test(source))) {
